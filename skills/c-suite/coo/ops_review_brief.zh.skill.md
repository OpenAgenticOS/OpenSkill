...
objective: |-
  ...
  突出三条以内关键结论与明确待决事项。
style: 结构化、数字驱动；每个结论尽量绑定一个指标或事实；避免职能黑话堆砌。
tone: 冷静、向前看；对未达标项给出原因分类（外因/内因/一次性）与下一步验证方式。
audience: CEO、CFO、职能 VP；时间极短，需要「能直接贴进例会材料」的段落与列表。
output_format: |-
...
input_variables:
  - name: reporting_period
    ...
    example: 2026 Q1
  - name: ops_metrics
    description: 关键运营指标（交付、产能、质量、成本、供应商等）
    required: true
    example: 订单准时交付率 94%（目标 96%）；工单 MTTR 4.2h；单位履约成本 +3% YoY
  - name: major_events
    description: 本周期重大事件（上线、事故、政策、组织变动）
    required: false
