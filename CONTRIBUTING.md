...
- **Recipe**：放在 **`recipes/<layer>/`**（`layer` 为 `c-suite` / `management` / `individual-contributor` / `cross-functional`，与技能库层级一致），成对添加 `*.zh.recipe.md` 与 `*.en.recipe.md`，frontmatter **`category`** 为 `layer` 或 `layer/role`（须与目录首段一致），同一 `id`（如 `recipe/my_entry`），符合 [schema/recipe.schema.json](./schema/recipe.schema.json)。面向阅读与导航，非可执行脚本。
...
- **Recipes:** under **`recipes/<layer>/`** with **`category`** `layer` or `layer/role` matching the folder’s first segment, paired `*.zh.recipe.md` + `*.en.recipe.md`, same `id` like `recipe/my_entry`, schema [recipe.schema.json](./schema/recipe.schema.json). Human-readable entry guides only.
