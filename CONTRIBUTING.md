...
- **工作流**：放在 **`workflows/<category>/`**（`category` 为流程域枚举，如 `strategy-planning`、`engineering-ops`），成对添加 `*.zh.workflow.md` 与 `*.en.workflow.md`，frontmatter **`category`** 须与目录名一致，同一 `id`（如 `workflow/my_flow`），符合 [schema/workflow.schema.json](./schema/workflow.schema.json)。步骤为**声明式**，不要求特定 Agent 框架。
- **Recipe**：放在 **`recipes/<layer>/`**（`layer` 为 `c-suite` / `management` / `individual-contributor` / `cross-functional`，与技能库层级一致），成对添加 `*.zh.recipe.md` 与 `*.en.recipe.md`，frontmatter **`category`** 为 `layer` 或 `layer/role`（须与目录首段一致），同一 `id`（如 `recipe/my_entry`），符合 [schema/recipe.schema.json](./schema/recipe.schema.json)。面向阅读与导航，非可执行脚本。读者如何在站内与外部工具中使用，见 [README.md](./README.md) 与 [使用指南](https://openagenticos.github.io/OpenSkill/zh/guide/) / [Usage guide](https://openagenticos.github.io/OpenSkill/en/guide/)。
...
- **Workflows:** under **`workflows/<category>/`** with **`category`** in frontmatter matching the folder (enum in schema), paired `*.zh.workflow.md` + `*.en.workflow.md`, same `id` like `workflow/my_flow`, schema [workflow.schema.json](./schema/workflow.schema.json). Steps are **declarative**; no runtime lock-in.
- **Recipes:** under **`recipes/<layer>/`** with **`category`** `layer` or `layer/role` matching the folder’s first segment, paired `*.zh.recipe.md` + `*.en.recipe.md`, same `id` like `recipe/my_entry`, schema [recipe.schema.json](./schema/recipe.schema.json). Human-readable entry guides only. For how readers use them on the site and in external tools, see [README.md](./README.md) and the [Usage guide](https://openagenticos.github.io/OpenSkill/en/guide/) / [使用指南](https://openagenticos.github.io/OpenSkill/zh/guide/).
